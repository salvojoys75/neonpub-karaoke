...
// FIX: Gestisce sia stringa che oggetto, e broadcast per display
export const sendReaction = async (data) => {
  ...
  const emoji = typeof data === 'string' ? data : (data?.emoji || data);
  ...
  // BROADCAST per display (fallback se postgres_changes non funziona sulla tabella reactions)
  try {
    const broadcastCh = supabase.channel('reactions_live_broadcast')
    broadcastCh.subscribe(async (status) => {
      if (status === 'SUBSCRIBED') {
        await broadcastCh.send({
          type: 'broadcast',
          event: 'new_reaction',
          payload: { emoji: emoji, nickname: nicknameToSend, id: reaction.id }
        })
        setTimeout(() => { try { supabase.removeChannel(broadcastCh) } catch(e){} }, 3000)
      }
    })
  } catch(e) { console.log('Broadcast reaction fallback error:', e) }
...
// FIX: Error checking per game update + broadcast vincitore
export const validateArcadeAnswer = async (bookingId, isCorrect, givenAnswer = null) => {
  try {
    ...
    if (!game) throw new Error('Gioco non trovato per questa prenotazione');
    ...
    if (isCorrect) {
      ...
      // 2. CHIUDI GIOCO E SETTA VINCITORE - CON ERROR CHECK
      const { error: gameUpdateError } = await supabase.from('arcade_games')
        .update({ 
          status: 'ended', 
      ...
      if (gameUpdateError) {
        console.error('ERRORE CRITICO: impossibile aggiornare gioco con vincitore:', gameUpdateError);
      }

      // 3. BROADCAST vincitore per il display (fallback se realtime non aggiorna)
      try {
        const winnerCh = supabase.channel('arcade_winner_broadcast')
        winnerCh.subscribe(async (status) => {
          if (status === 'SUBSCRIBED') {
            await winnerCh.send({
              type: 'broadcast',
              event: 'arcade_winner',
            ...
            setTimeout(() => { try { supabase.removeChannel(winnerCh) } catch(e){} }, 3000)
          }
        })
      } catch(e) { console.log('Broadcast winner error:', e) }
    }
