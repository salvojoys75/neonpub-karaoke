...
export const api = {
  ...
  getPub: async (code) => supabase.from('events').select('*').eq('code', code?.toUpperCase()).single(),

  getActiveEventsForUser: async () => {
    const { data: user } = await supabase.auth.getUser()
    if (!user?.user) return []
    const { data } = await supabase.from('events').select('*').eq('owner_id', user.user.id).eq('status', 'active').gt('expires_at', new Date().toISOString())
    return data || []
  },

  joinPub: async ({ pub_code, nickname }) => {
    const { data: event } = await supabase.from('events').select('*').eq('code', pub_code.toUpperCase()).single()
    if (!event || event.status !== 'active') throw new Error("Evento non attivo")
  ...
  createOperatorProfile: async (email, name, password) => supabase.functions.invoke('create-user', { body: { email, password, name } }),

  // --- KARAOKE & REGIA ---
  getEventState: async () => {
    const code = localStorage.getItem('neonpub_pub_code')
    if (!code) return null
    const { data } = await supabase.from('events').select('active_module, active_module_id').eq('code', code).maybeSingle()
    return data
  },
  ...
  getAdminCurrentPerformance: async () => {
    const e = await getAdminEvent()
  ...
  startPerformance: async (reqId, ytUrl) => {
  ...
  endQuiz: async (id) => {
    ...
    const code = localStorage.getItem('neonpub_pub_code')
    return supabase.from('events').update({ active_module: 'karaoke' }).eq('code', code)
  },
  ...
  getAdminPendingMessages: async () => {
    const e = await getAdminEvent()
    const { data } = await supabase.from('messages').select('*, participants(nickname)').eq('event_id', e.id).eq('status', 'pending')
