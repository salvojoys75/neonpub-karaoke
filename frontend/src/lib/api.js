...
export const getSongQueue = async () => {
  const participant = getParticipantFromToken()
  const { data, error } = await supabase.from('song_requests').select(`*, participants (nickname)`).eq('event_id', participant.event_id).in('status', ['pending', 'queued', 'approved']).order('position', { ascending: true })
  if (error) throw error
...
export const pausePerformance = async (performanceId) => {
  const { data, error } = await supabase.from('performances').update({ status: 'paused' }).eq('id', performanceId).select(); if (error) throw error; return { data };
}
export const resumePerformance = async (performanceId) => {
  const { data, error } = await supabase.from('performances').update({ status: 'live' }).eq('id', performanceId).select(); if (error) throw error; return { data };
}
export const restartPerformance = async (performanceId) => {
  const { error } = await supabase.from('performances').update({ status: 'restarted', started_at: new Date().toISOString() }).eq('id', performanceId); if (error) throw error;
  await supabase.from('performances').update({ status: 'live' }).eq('id', performanceId); return { data: 'ok' };
}
export const skipPerformance = async (performanceId) => {
  const { data, error } = await supabase.from('performances').update({ status: 'ended' }).eq('id', performanceId).select(); if (error) throw error; return { data };
}
export const togglePerformanceMute = async (performanceId, isMuted) => {
  const { data, error } = await supabase.from('performances').update({ is_muted: isMuted }).eq('id', performanceId).select(); if (error) throw error; return { data };
}
export const togglePerformanceBlur = async (performanceId, isBlurred) => {
  const { data, error } = await supabase.from('performances').update({ is_blurred: isBlurred }).eq('id', performanceId).select(); if (error) throw error; return { data };
}
...
export const submitVote = async (data) => {
  const participant = getParticipantFromToken()
  const { data: vote, error } = await supabase.from('votes').insert({ performance_id: data.performance_id, participant_id: participant.participant_id, score: data.score }).select().single()
...
export const sendReaction = async (data) => {
  const participant = getParticipantFromToken()
  const { data: reaction, error } = await supabase.from('reactions').insert({ event_id: participant.event_id, participant_id: participant.participant_id, emoji: data.emoji, nickname: participant.nickname }).select().single()
  if (error) throw error; return { data: reaction }
}

export const sendMessage = async (data) => {
  let participantId = null, eventId = null, status = 'pending';
  try { const p = getParticipantFromToken(); participantId = p.participant_id; eventId = p.event_id; } 
  catch (e) { 
      const pubCode = localStorage.getItem('neonpub_pub_code');
      if(pubCode) { const { data: ev } = await supabase.from('events').select('id').eq('code', pubCode).single(); if(ev) { eventId = ev.id; status = data.status || 'pending'; } }
  }
  if (!eventId) throw new Error("Errore contesto evento");
  const text = typeof data === 'string' ? data : (data.text || data.message);
  const { data: message, error } = await supabase.from('messages').insert({ event_id: eventId, participant_id: participantId, text: text, status: status }).select().single()
...
export const importQuizBatch = async (jsonData) => { const { data, error } = await supabase.from('quiz_library').insert(jsonData).select(); if (error) throw error; return { data }; }

export const getLibraryQuizzes = async () => { const { data, error } = await supabase.from('quiz_library').select('*').limit(50); if (error) throw error; return { data }; }
...
export const launchQuizFromLibrary = async (libraryId) => {
  ...
  if (!template) throw new Error("Quiz non trovato");
  
  let safeOptions = template.options;
  if (typeof safeOptions === 'string') { try { safeOptions = JSON.parse(safeOptions); } catch (e) { safeOptions = ["A", "B", "C", "D"]; } }
  
  const { data: activeQuiz, error } = await supabase.from('quizzes').insert({
      event_id: event.id, category: template.category, question: template.question, options: safeOptions,
      correct_index: template.correct_index, media_url: template.media_url, points: template.points, status: 'active'
    }).select().single();
  if (error) throw error; return { data: activeQuiz };
}

// FIX: Aggiunto media_url anche nel quiz manuale
export const startQuiz = async (data) => {
  ...
  const { data: quiz, error } = await supabase.from('quizzes').insert({
    ...
    correct_index: data.correct_index, points: data.points, media_url: data.media_url || null, // <--- FIX QUI
    status: 'active'
  }).select().single()
  if (error) throw error; return { data: quiz }
}

export const closeQuizVoting = async (quizId) => { const { data, error } = await supabase.from('quizzes').update({ status: 'closed' }).eq('id', quizId).select(); if (error) throw error; return { data }; }
export const showQuizResults = async (quizId) => { const { data, error } = await supabase.from('quizzes').update({ status: 'showing_results' }).eq('id', quizId).select().single(); if (error) throw error; return { data } }
...
export default {
  ...
  togglePerformanceMute, togglePerformanceBlur, getCurrentPerformance, getAdminCurrentPerformance,
  ...
  importQuizBatch, getLibraryQuizzes, launchQuizFromLibrary,
  getQuizResults, getQuizLeaderboard, getLeaderboard, getAdminLeaderboard, getDisplayData
}