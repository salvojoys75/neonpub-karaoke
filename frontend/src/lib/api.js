...
export const getDisplayData = async (pubCode) => {
  ...
  const [perf, queue, lb, activeQuiz, adminMsg, approvedMsgs, activeArcade, activeMill] = await Promise.all([
    ...
    supabase.from('song_requests').select('*, participants(nickname, avatar_url)').eq('event_id', event.id).eq('status', 'queued').order('position', {ascending: true}).limit(20),
...
export const getQuizResults = async (quizId) => {
  ...
  const { data: answers } = await supabase.from('quiz_answers').select('*, participants(id, nickname, avatar_url)').eq('quiz_id', quizId)
...
export const getAdminLeaderboard = async () => {
  const event = await getAdminEvent()
  const { data } = await supabase.from('participants').select('id, nickname, score').eq('event_id', event.id).order('score', {ascending:false}).limit(20)
  return { data }
}
...
export const updateProfileCredits = async (id, credits) => { const val = parseInt(credits, 10); await supabase.from('profiles').update({ credits: val }).eq('id', id); return { data: 'ok' }; }
...
export const createOperatorProfile = async (email, name, password, initialCredits) => {
    ...
    const { data } = await supabase.functions.invoke('create-user', { body: { email, password, name, credits: initialCredits }, headers: { Authorization: `Bearer ${session?.access_token}` } });
    return data;
}
...
export const setEventModule = async (moduleId, specificContentId = null) => {
  ...
  if (moduleId === 'quiz' && specificContentId) {
    ...
    if (catalogItem) {
        await supabase.from('quizzes').update({ status: 'ended' }).eq('event_id', event.id).neq('status', 'ended');
        await supabase.from('quizzes').insert({ event_id: event.id, category: catalogItem.category, question: catalogItem.question, options: catalogItem.options, correct_index: catalogItem.correct_index, points: catalogItem.points, status: 'active', media_url: catalogItem.media_url || null, media_type: catalogItem.media_type || 'text' });
    }
  }
};
export const getQuizCatalog = async (venueId = null, daysThreshold = 30) => {
  ...
  if (venueId && catalog) {
      ...
      catalog = catalog.map(q => ({...q, recently_used: usedIds.has(q.id)}));
  }
  return { data: catalog || [] };
};
...
export const loadQuizModule = async (moduleId) => {
    ...
    for (const q of questions) {
        const { data: inserted } = await supabase.from('quiz_catalog').insert({ category: q.category || module.category, question: q.question, options: q.options, correct_index: q.correct_index, points: q.points, media_url: q.media_url, media_type: q.media_type }).select('id').single();
        if (inserted) await supabase.from('event_quiz_catalog').upsert({ event_id: event.id, quiz_catalog_id: inserted.id }, { onConflict: 'event_id,quiz_catalog_id', ignoreDuplicates: true });
    }
...
// ─── BAND MODE API (SISTEMATO) ────────────────────────────────────────────────

/**
 * Avvia la sessione band.
 * FIX: Usa un singolo broadcast unificato per evitare race condition.
 * Il payload contiene sia setup che start info insieme.
 */
export const startBandSession = async (songId, songTitle, assignments) => {
  const event = await getAdminEvent();

  // Resetta altri moduli in parallelo
  await Promise.all([
  ...
  // Genera un ID univoco per la sessione e timestamp server
  const sessionId = `${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
  const serverTime = Date.now();
  
  // active_band contiene tutto il necessario per recovery dopo reload
  const activeBand = {
    status: 'active',
    song: songId,
    songTitle: songTitle,
  ...
  // Salva nel DB PRIMA del broadcast (così i client che ricaricano possono recuperare)
  await supabase.from('events').update({
    active_module: 'band',
    active_band: activeBand,
  }).eq('id', event.id);

  // Broadcast unificato con timeout
  const broadcastPromise = new Promise((resolve) => {
    const channel = supabase.channel(`band_game_${event.code}`);
    const timeout = setTimeout(() => {
      try { supabase.removeChannel(channel); } catch(e) {}
      resolve({ broadcast: false, reason: 'timeout' });
    }, 5000);
    ...
    channel.subscribe(async (status) => {
      ...
      try {
        // FIX: Singolo evento unificato - elimina race condition
        await channel.send({
          type: 'broadcast',
          event: 'band_init',
          payload: {
        ...
        clearTimeout(timeout);
        try { supabase.removeChannel(channel); } catch(e) {}
        resolve({ broadcast: false, reason: err.message });
      }
    });
  });
...
/**
 * Ferma la sessione band.
 */
export const stopBandSession = async () => {
  const event = await getAdminEvent();

  // Aggiorna DB prima del broadcast
  await supabase.from('events').update({
    active_module: 'karaoke',
    active_band: null,
  }).eq('id', event.id);
...
export default {
    ...
    createArcadeGame, startArcadeGame, pauseArcadeGame, resumeArcadeGame, endArcadeGame, bookArcadeAnswer, validateArcadeAnswer, getArcadeBookings, getCurrentBooking, cancelArcadeBooking, getArcadeLeaderboard,
