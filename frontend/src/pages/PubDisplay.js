...
// ===========================================
// QUIZ OVERLAY (testo) – invariato rispetto al tuo originale
// ===========================================
    ...
    return (
      <div className="absolute inset-0 bg-zinc-900 z-50 flex flex-col p-8 overflow-hidden animate-fade-in">
        <div className="text-center mb-6">
          <h1 className="text-6xl font-black text-yellow-500 uppercase drop-shadow-[0_0_15px_rgba(234,179,8,0.5)]">
            CLASSIFICA GENERALE
          </h1>
        </div>
        <div className="flex-1 overflow-y-auto grid grid-cols-2 gap-x-12 gap-y-4 px-12 content-start custom-scrollbar">
          {leaderboard.map((p, i) => (
            <div
              key={p.id}
              className={`flex items-center p-4 rounded-xl text-3xl font-bold ${
                i < 3
                  ? "scale-105 bg-gradient-to-r from-yellow-600/30 to-transparent border border-yellow-500/50"
                  : "bg-white/5"
              }`}
            >
              <span
                className={`w-16 h-16 flex items-center justify-center rounded-full mr-6 text-2xl border-4 ${
                  i === 0 ? "bg-yellow-500 text-black border-yellow-300" : "bg-zinc-800 text-zinc-500 border-zinc-600"
                }`}
              >
                {i + 1}
              </span>
      ...
      <div className="absolute inset-0 z-30 flex flex-col justify-between pointer-events-none">
        <div className="bg-black/80 backdrop-blur-md p-6 border-b border-white/10 animate-slide-down">
          <div className="flex justify-between items-center mb-2">
            <span
              className={`px-4 py-1 rounded text-xl font-bold uppercase tracking-widest ${
                quiz.status === "closed" ? "bg-red-600" : "bg-fuchsia-600 animate-pulse"
              }`}
            >
        ...
        <div className="bg-gradient-to-t from-black via-black/90 to-transparent p-8 pb-12 animate-slide-up">
          <div className="grid grid-cols-4 gap-6 max-w-[95%] mx-auto">
            {quiz.options.map((opt, i) => (
              <div
                key={i}
                className={`p-6 rounded-2xl text-3xl font-bold border-2 text-center ${
                  quiz.status === "closed"
  ...
  return (
    <div className="absolute inset-0 z-30 flex flex-col items-center justify-center p-10 bg-gradient-to-b from-purple-900/90 to-black/90">
      <div className="w-full max-w-6xl text-center animate-zoom-in">
        <div className="mb-8">
          <span
            className={`px-12 py-4 rounded-full text-4xl font-black uppercase tracking-widest shadow-[0_0_30px_rgba(217,70,239,0.6)] ${
        ...
        <div className="grid grid-cols-2 gap-8">
          {quiz.options.map((opt, i) => (
            <div
              key={i}
              className={`p-8 rounded-3xl text-5xl font-bold border-4 ${
                quiz.status === "closed"
                  ? "border-zinc-700 text-zinc-500 bg-black/60"
...
export default function PubDisplay() {
  ...
  const loadDisplayData = useCallback(async () => {
    try {
      ...
      if (data) {
        ...
        if (
          data.current_performance?.status === "ended" &&
          !voteResult &&
          data.current_performance.average_score > 0
        ) {
      ...
      console.error("Errore caricamento display data:", error);
    }
  }, [pubCode, voteResult]);
  ...
  useEffect(() => {
    ...
    const controlChannel = supabase
      .channel(`display_control_${pubCode}`)
      .on("broadcast", { event: "control" }, (payload) => {
        ["karaoke-player", "quiz-fixed-player"].forEach((pid) => {
          const player = window.YT?.get?.(pid);
          if (player?.mute) {
            payload.payload.value ? player.mute() : player.unMute();
          }
        });
      })
      .subscribe();
    ...
    const channel = supabase
      ...
      .on(
        ...
        async (payload) => {
          if (payload.new.status === "approved") showFlashMessage({ text: payload.new.text, nickname: "Regia" });
        }
      )
      .on(
        "postgres_changes",
  ...
  const isKaraokeActive =
    !isQuizVisible &&
    currentPerf &&
    (currentPerf.status === "live" || currentPerf.status === "paused" || currentPerf.status === "voting" || voteResult);
          ...
          <div className="w-[350px] bg-zinc-900/95 border-l border-zinc-800 flex flex-col z-30 shadow-2xl relative">
            ...
            <div className="flex-1 overflow-hidden flex flex-col p-8 items-center justify-center text-center space-y-6">
              {displayData?.pub?.logo_url ? (
                <img
                  src={displayData.pub.logo_url}
                  alt="Logo"
                  className="w-40 h-40 object-contain drop-shadow-2xl"
                />
              ) : (
                <div className="w-40 h-40 bg-zinc-800 rounded-full flex items-center justify-center text-zinc-600 text-4xl font-bold border-4 border-zinc-700">
                  LOGO
                </div>
              )}
              <div className="mt-2">
                <h2 className="text-3xl font-black text-white uppercase tracking-wider">
                  {displayData?.pub?.name || "NEONPUB"}
                </h2>
              </div>
            </div>
      ...
      {flashMessages.length > 0 && (
        <div className="fixed top-24 left-8 z-[110] w-2/3 max-w-4xl flex flex-col gap-4">
          {flashMessages.map((msg) => (
            <div
              key={msg.internalId}
              className="bg-black/90 backdrop-blur-xl border-l-8 border-cyan-500 text-white p-6 rounded-r-2xl shadow-2xl animate-slide-in-left"
            >
