...
export default function PubDisplay() {
    ...
    const load = useCallback(async () => {
        try {
            ...
            if(res.data) {
                ...
                // METODO 1: Controlla winner_id sul gioco
                if (arcade && arcade.status === 'ended' && arcade.winner_id) {
                    try {
                        ...
                        console.error('Errore fetch vincitore da winner_id:', e);
                    }
                }

                // METODO 2 (FALLBACK): Se non c'è arcade_result ma c'è un booking 'correct', usa quello come vincitore
                if (!finalData.arcade_result && arcade && arcade.id) {
                    try {
                        const { data: correctBookings } = await supabase
                            .from('arcade_bookings')
                            .select('*, participants:participant_id(id, nickname, avatar_url)')
                            .eq('game_id', arcade.id)
                            .eq('status', 'correct')
                            .order('validated_at', { ascending: false })
                            .limit(1);
                        
                        if (correctBookings && correctBookings.length > 0 && correctBookings[0].participants) {
                            finalData = {
                                ...finalData,
                                arcade_result: { winner: correctBookings[0].participants }
                            };
                            // Forza lo status ended se c'è un vincitore ma lo status non è aggiornato
                            if (finalData.active_arcade && finalData.active_arcade.status !== 'ended') {
                                finalData = {
                                    ...finalData,
                                    active_arcade: {
                                        ...finalData.active_arcade,
                                        status: 'ended'
                                    }
                                };
                            }
                        ...
                        console.error('Errore fallback vincitore da bookings:', e);
                    }
                }

                // Coda prenotazioni e ultimo errore (solo se gioco attivo e NON c'è già un vincitore)
                if (arcade && arcade.status === 'active' && !finalData.arcade_result) {
                    try {
    ...
    // POLLING REACTIONS (fallback se postgres_changes non funziona)
    useEffect(() => {
        ...
        const pollReactions = async () => {
            try {
                const { data: latest } = await supabase
                    .from('reactions')
                    .select('id, emoji, nickname, created_at')
                    .eq('event_id', data.pub.id)
                    .order('created_at', { ascending: false })
                    .limit(1)
                    .maybeSingle();
                
                if (latest && latest.id !== lastReactionIdRef.current) {
        ...
        const reactionPollInterval = setInterval(pollReactions, 2000);
        return () => clearInterval(reactionPollInterval);
    }, [data?.pub?.id]);
    ...
    useEffect(() => {
        ...
        // Canale principale per controlli e postgres_changes
        const ch = supabase.channel('tv_ctrl')
            ...
            .on('postgres_changes', {event: 'INSERT', schema: 'public', table: 'reactions'}, p => {
                console.log(\"REACTION RECEIVED via postgres_changes:\", p.new);
        ...
        // Canale broadcast per reactions (fallback affidabile)
        const reactionsBroadcastCh = supabase.channel('reactions_live_broadcast')
            .on('broadcast', {event: 'new_reaction'}, p => {
                console.log(\"REACTION RECEIVED via broadcast:\", p.payload);
                if (p.payload && p.payload.emoji) {
                    if (p.payload.id) lastReactionIdRef.current = p.payload.id;
                    setNewReaction({
                        emoji: p.payload.emoji,
                        nickname: p.payload.nickname || '',
                        id: p.payload.id || Date.now(),
                        _t: Date.now()
                    });
                }
            })
        ...
        // Canale broadcast per vincitore arcade (fallback)
        const arcadeWinnerCh = supabase.channel('arcade_winner_broadcast')
            .on('broadcast', {event: 'arcade_winner'}, p => {
                console.log(\"ARCADE WINNER BROADCAST:\", p.payload);
                load();
            })
            .subscribe();
        ...
        return () => { 
            ...
            supabase.removeChannel(reactionsBroadcastCh);
            supabase.removeChannel(arcadeWinnerCh);
        };
    }, [pubCode, load]);
