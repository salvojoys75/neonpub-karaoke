...
const KaraokeScreen = memo(({ performance, isVoting, voteResult }) => {
    ...
    useEffect(() => {
        ...
        if (!window.YT) { /* API loaded elsewhere usually */ }
            ...
            {!isVoting && !voteResult && (
                <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black via-black/80 to-transparent p-12 z-10 pb-20 animate-fade-in">
                    ...
                    <div className="flex items-end gap-6">
                        ...
                        <div className="bg-fuchsia-600 px-6 py-2 rounded-full flex items-center gap-3 animate-pulse"><Mic2 className="w-8 h-8" /><span className="text-2xl font-bold uppercase tracking-wider">{performance.user_nickname}</span></div>
                    </div>
                </div>
            )}
            {isVoting && (
...
// ===========================================
// QUIZ SCREEN (ALWAYS MOUNTED MEDIA)
// ===========================================
const QuizScreen = memo(({ quiz, quizResults, leaderboard }) => {
    ...
    return (
        <div className="absolute inset-0 bg-black z-40 flex flex-col items-center justify-center overflow-hidden">
            {/* LAYER 0: MEDIA (Sempre montato, isResult controlla solo opacity/z-index internamente) */}
            <QuizMediaFixed 
                            ...
                            {!isResults ? (
                                ...
                                <div className="animate-zoom-in">
                                    ...
                                    <div className="bg-black/70 backdrop-blur-md p-10 rounded-[3rem] border border-white/10 shadow-2xl">
                                        ...
                                        <div className="grid grid-cols-2 gap-8 mb-12">{quiz.options.map((opt, i) => {
...
export default function PubDisplay() {
  ...
  useEffect(() => {
    ...
    const controlChannel = supabase.channel(`display_control_${pubCode}`).on('broadcast', { event: 'control' }, (p) => {
            if(p.payload.command === 'mute') {
                const ytKaraoke = window.YT?.get && window.YT.get('karaoke-player');
                if (ytKaraoke?.mute) p.payload.value ? ytKaraoke.mute() : ytKaraoke.unMute();
            }
        }).subscribe();
    const channel = supabase.channel(`display_realtime`)
        ...
        .on('postgres_changes', { event: '*', schema: 'public', table: 'song_requests', filter: `event_id=eq.${displayData.pub.id}` }, async () => {
             const { data: queueData } = await supabase.from('song_requests').select('*, participants(nickname)').eq('event_id', displayData.pub.id).eq('status', 'queued').limit(10);
             const queue = queueData?.map(q => ({...q, user_nickname: q.participants?.nickname})) || [];
             setDisplayData(prev => ({ ...prev, queue }));
             if (queue.length > 0) setTicker(queue.slice(0, 5).map((s, i) => `${i + 1}. ${s.title} (${s.user_nickname})`).join(' • '));
             else setTicker("Inquadra il QR Code per cantare!");
        })
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'reactions', filter: `event_id=eq.${displayData.pub.id}` }, (p) => {
            const id = Date.now() + Math.random();
            setFloatingReactions(prev => [...prev, { id, emoji: p.new.emoji, nickname: p.new.nickname, left: Math.random() * 80 + 10 }]);
            setTimeout(() => setFloatingReactions(prev => prev.filter(r => r.id !== id)), 4000);
        })
        .on('postgres_changes', { event: '*', schema: 'public', table: 'messages', filter: `event_id=eq.${displayData.pub.id}` }, async (p) => {
             if(p.new.status === 'approved') {
                 let nick = "Regia";
                 if(p.new.participant_id) {
                     const { data } = await supabase.from('participants').select('nickname').eq('id', p.new.participant_id).single();
                     if(data) nick = data.nickname;
                 }
                 const id = Date.now();
                 setFlashMessages(prev => [...prev, { text: p.new.text, nickname: nick, internalId: id }]);
                 setTimeout(() => setFlashMessages(prev => prev.filter(m => m.internalId !== id)), 10000);
             }
        })
        .on('postgres_changes', { event: '*', schema: 'public', table: 'participants', filter: `event_id=eq.${displayData.pub.id}` }, async () => {
            const { data } = await supabase.from('participants').select('id, nickname, score').eq('event_id', displayData.pub.id).order('score', { ascending: false }).limit(20);
            setDisplayData(prev => ({ ...prev, leaderboard: data || [] }));
        })
        .on('postgres_changes', { event: '*', schema: 'public', table: 'quizzes', filter: `event_id=eq.${displayData.pub.id}` }, async (p) => {
            const updatedQuiz = p.new;
            setDisplayData(prev => {
                const current = prev.active_quiz;
                if (!current) return { ...prev, active_quiz: updatedQuiz };
                if (current.status !== updatedQuiz.status || current.media_state !== updatedQuiz.media_state || current.media_url !== updatedQuiz.media_url || current.question !== updatedQuiz.question) {
                    return { ...prev, active_quiz: updatedQuiz };
                }
                return prev;
            });
            if (updatedQuiz.status === 'showing_results') {
                const res = await api.getQuizResults(updatedQuiz.id); 
                setQuizResults(res.data);
            } else if (updatedQuiz.status === 'active' || updatedQuiz.status === 'closed') { setQuizResults(null); }
            else if (updatedQuiz.status === 'ended') { setTimeout(() => { setDisplayData(prev => ({ ...prev, active_quiz: null })); setQuizResults(null); }, 5000); }
        })
        .subscribe();
    ...
    <div className="h-screen bg-black text-white overflow-hidden flex flex-col font-sans">
      ...
      <div className="flex-1 flex overflow-hidden relative">
        <div className="flex-1 relative bg-black flex flex-col justify-center overflow-hidden">
           <div className={`absolute inset-0 transition-opacity duration-500 ${showQuiz ? 'opacity-100 z-50 pointer-events-auto' : 'opacity-0 z-0 pointer-events-none'}`}>{activeQuiz && (<QuizScreen quiz={activeQuiz} quizResults={quizResults} leaderboard={displayData?.leaderboard || []} />)}</div>
           <div className={`absolute inset-0 transition-opacity duration-500 ${showKaraoke ? 'opacity-100 z-40 pointer-events-auto' : 'opacity-0 z-0 pointer-events-none'}`}>{currentPerf && (<KaraokeScreen performance={currentPerf} isVoting={currentPerf.status === 'voting'} voteResult={voteResult} />)}</div>
           <div className={`absolute inset-0 transition-opacity duration-500 ${showWaiting ? 'opacity-100 z-30 pointer-events-auto' : 'opacity-0 z-0 pointer-events-none'}`}>
      ...
      <div className="reactions-overlay pointer-events-none fixed inset-0 z-[100] overflow-hidden">{floatingReactions.map(r => (<div key={r.id} className="absolute flex flex-col items-center animate-float-up" style={{ left: `${r.left}%`, bottom: '-50px' }}><span className="text-7xl filter drop-shadow-2xl">{r.emoji}</span><span className="text-xl text-white font-bold mt-1 bg-black/70 px-4 py-1 rounded-full border border-white/20 shadow-xl">{r.nickname}</span></div>))}</div>
      {flashMessages.length > 0 && (<div className="fixed top-24 left-8 z-[110] w-2/3 max-w-4xl flex flex-col gap-4 pointer-events-none">{flashMessages.map(msg => (<div key={msg.internalId} className="bg-black/90 backdrop-blur-xl border-l-8 border-cyan-500 text-white p-6 rounded-r-2xl shadow-2xl animate-slide-in-left flex items-start gap-6"><div className="bg-cyan-500/20 p-4 rounded-full"><MessageSquare className="w-10 h-10 text-cyan-400" /></div><div><p className="text-sm text-cyan-400 font-bold uppercase tracking-widest mb-1">{msg.nickname === 'Regia' ? '📢 MESSAGGIO DALLA REGIA' : `Messaggio da ${msg.nickname}`}</p><p className="text-4xl font-bold leading-tight">{msg.text}</p></div></div>))}</div>)}
