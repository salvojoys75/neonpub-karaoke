...
const KaraokeScreen = ({ performance, isVoting, voteResult }) => {
    ...
    useEffect(() => {
        ...
        if (isVoting || voteResult) {
            // Nascondi e pausa durante votazione
        ...
        const createOrUpdatePlayer = async () => {
            ...
            // Se è lo stesso video e player esiste, gestisci solo play/pause/restart
...
// ===========================================
// COMPONENTE: QUIZ SCREEN (CON FIX VIDEO)
// ===========================================
const QuizScreen = ({ quiz, quizResults, leaderboard }) => {
    // Memoizza quiz ID per evitare re-render del media
    const quizIdRef = useRef(null);
    
    // Aggiorna ref solo se cambia quiz ID
...
export default function PubDisplay() {
  ...
  useEffect(() => {
    ...
    // MUTE GLOBAL - FIX: usa refs globali invece di window.YT.get()
        ...
        .on('postgres_changes', { event: '*', schema: 'public', table: 'quizzes', filter: `event_id=eq.${displayData.pub.id}` },
            async (payload) => {
                ...
                if (updatedQuiz.status === 'active' || updatedQuiz.status === 'closed') {
                    // Se è un nuovo quiz, reset risultati
