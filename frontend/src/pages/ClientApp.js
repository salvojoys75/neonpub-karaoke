    ...
    <div className="min-h-screen bg-[#050505] flex flex-col pb-24 font-sans text-white">
      ...
      <header className="sticky top-0 z-40 bg-[#050505]/90 backdrop-blur-md p-4 flex justify-between items-center border-b border-white/5">
        ...
        <div className="flex items-center gap-2">
          <Button onClick={() => { loadData(); toast.success("Aggiornato!"); }} variant="ghost" size="sm" className="text-zinc-400">
            ...
            {currentPerformance ? (
              <div className="glass rounded-2xl p-5 neon-border bg-gradient-to-br from-fuchsia-900/20 to-black">
                ...
                <div className="flex items-center gap-2 bg-white/5 p-2 rounded-lg mb-4">
                   ...
                   <span className="text-fuchsia-400 font-medium">{currentPerformance.user_nickname}</span>
                </div>
      ...
      <Dialog open={showVoteModal} onOpenChange={setShowVoteModal}>
        <DialogContent className="bg-zinc-900 border-zinc-800 text-center">
          ...
          <div className="flex justify-center gap-2 py-4">
            {[1, 2, 3, 4, 5].map(star => (
              <button key={star} onClick={() => setSelectedStars(star)}>
                <Star className={`w-10 h-10 ${selectedStars >= star ? 'text-yellow-500 fill-yellow-500' : 'text-zinc-600'}`} />
              </button>
            ))}
          </div>
