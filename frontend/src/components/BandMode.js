...
export default function BandMode({ session, pubCode, chart: chartProp }) {
  ...
  useEffect(() => {
    ...
    channel.on('broadcast', { event: 'band_hit' }, ({ payload }) => {
      ...
      setPlayers(p => { const pl = p[nickname] || { score: 0, combo: 0 }; return { ...p, [nickname]: { score: pl.score + points, combo: pl.combo + 1 } }; });
    });
    channel.on('broadcast', { event: 'band_miss' }, ({ payload }) => {
      setPlayers(p => { const pl = p[payload.nickname] || { score: 0, combo: 0 }; return { ...p, [payload.nickname]: { ...pl, combo: 0 } }; });
      setCombo(0);
    });
    channel.subscribe();
      ...
      <div style={{ position: 'relative', overflow: 'hidden', display: 'flex' }}>
        ...
        {gameState === 'waiting' && chart && (
          <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.75)', zIndex: 20 }}>
            ...
            <button onClick={startSequence} style={{ padding: '18px 48px', fontSize: '22px', fontWeight: 900, background: '#ffd100', color: '#000', border: 'none', borderRadius: '12px', cursor: 'pointer', boxShadow: '0 0 40px rgba(255,209,0,0.5)' }}>▶ START</button>
          </div>
        )}
      </div>
