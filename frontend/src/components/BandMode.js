...
const START_DELAY = 10000; // Allineato con API (10s)
const DEFAULT_BPM = 126;
const HIT_SUSTAIN = 0.5;

const INSTRUMENT_COLORS = { keys: '#00d4ff', drums: '#ff3b5c', bass: '#39ff84', brass: '#ffd100', guitar: '#ff8c00' };
const INSTRUMENT_ICONS = { keys: '🎹', drums: '🥁', bass: '🎸', brass: '🎺', guitar: '🎸' };
const INSTRUMENT_AUDIO = { keys: 'organo.mp3', drums: 'drums.mp3', bass: 'bass.mp3', brass: 'brass.mp3', guitar: 'guitar.mp3' };
...
function Spotlight({ player, instrument, color, icon, side }) {
  ...
  useEffect(() => { energyPropRef.current = player?.energy || 0; }, [player?.energy]);
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      if (fillH > 1) {
        ...
        ctx.beginPath(); ctx.roundRect(barX, barY + barH - fillH, barW, fillH, 3); ctx.fill(); ctx.shadowBlur = 0;
      }
...
function KaraokeDisplay({ lyrics, elapsed, songTitle, songArtist, gameState, countdown }) {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', width:'100%', height:'100%', padding:'16px', textAlign:'center' }}>
      ...
      {gameState === 'countdown' && <div style={{ fontSize:'clamp(80px,14vw,180px)', fontWeight:900, color:'#fff', lineHeight:1 }}>{countdown}</div>}
      {gameState === 'playing' && (
        <>
          <div style={{ position:'relative', display:'inline-block', marginBottom:'12px' }}>
            <div style={{ fontSize:'clamp(28px,5vw,72px)', fontWeight:900, color:'rgba(255,255,255,0.18)', letterSpacing:'0.04em', lineHeight:1.15, fontFamily:"'JetBrains Mono',monospace" }}>{activeLine?.text || '♪'}</div>
            {activeLine && (
              <div style={{ position:'absolute', top:0, left:0, overflow:'hidden', width:`${progress*100}%`, whiteSpace:'nowrap' }}>
                <div style={{ fontSize:'clamp(28px,5vw,72px)', fontWeight:900, color:'#fff', letterSpacing:'0.04em', lineHeight:1.15, fontFamily:"'JetBrains Mono',monospace", textShadow:'0 0 30px rgba(255,255,255,0.8), 0 0 60px rgba(255,255,255,0.4)' }}>{activeLine.text}</div>
              </div>
            )}
          </div>
          {nextLine && <div style={{ fontSize:'clamp(16px,2.5vw,36px)', fontWeight:700, color:'rgba(255,255,255,0.25)', letterSpacing:'0.04em', fontFamily:"'JetBrains Mono',monospace" }}>{nextLine.text}</div>}
...
export default function BandMode({ session, pubCode }) {
  ...
  useEffect(() => {
    fetch(`/audio/${song}/manifest.json`).then(r => r.json()).then(m => setSongMeta({ title: m.title || song, artist: m.artist || '' })).catch(() => {});
    fetch(`/audio/${song}/lyrics.json`).then(r => r.json()).then(setLyrics).catch(() => setLyrics([]));
  }, [song]);
  ...
  const getServerTime = async () => {
    try {
      ...
      if (dateStr) {
        timeOffsetRef.current = new Date(dateStr).getTime() + ((end - start) / 2) - end;
      }
    } catch {}
  };

  useEffect(() => { getServerTime(); }, []);
  ...
  const setupAudio = useCallback(async (ass) => {
    ...
    ass.forEach(a => {
      ...
      el.crossOrigin = 'anonymous'; el.preload = 'auto'; el.loop = false;
  ...
  const startSequence = useCallback(async (ass, startAtISO) => {
    if (gameStateRef.current !== 'waiting') return;
    await setupAudio(ass);
    const ctx = audioCtxRef.current;

    const targetTime = new Date(startAtISO).getTime();
    const nowServer = Date.now() + timeOffsetRef.current;
    const msDelay = Math.max(0, targetTime - nowServer);
    ...
    channel.on('broadcast', { event: 'band_hit' }, ({ payload }) => {
      ...
      setPlayers(prev => {
        ...
        return { ...prev, [nickname]: {
          ...pl, score: pl.score + (points || 0), combo: pl.combo + 1,
  ...
  return (
    <div style={{ width:'100%', height:'100%', background:'#06060e', display:'grid', gridTemplateColumns: leftPlayers.length > 0 ? '18% 1fr 18%' : '0 1fr 0', gridTemplateRows:'1fr auto', fontFamily:"'JetBrains Mono',monospace", color:'#fff', overflow:'hidden' }}>
      <div style={{ display:'flex', flexDirection:'column', borderRight: leftPlayers.length > 0 ? '1px solid rgba(255,255,255,0.05)' : 'none', background:'rgba(0,0,0,0.3)', overflow:'hidden' }}>
        {leftPlayers.map(a => (<div key={a.nickname} style={{ flex:1, borderBottom:'1px solid rgba(255,255,255,0.04)' }}><Spotlight player={players[a.nickname]} instrument={a.instrument} color={INSTRUMENT_COLORS[a.instrument]||'#fff'} icon={INSTRUMENT_ICONS[a.instrument]||'🎵'} side="left" /></div>))}
      </div>
      <div style={{ position:'relative', overflow:'hidden' }}>
        ...
        {gameState === 'playing' && totalScore > 0 && <div style={{ position:'absolute', top:12, left:'50%', transform:'translateX(-50%)', fontSize:'clamp(10px,1.2vw,16px)', color:'rgba(255,209,0,0.6)', fontWeight:900, letterSpacing:'0.2em', zIndex:5 }}>BANDA: {totalScore.toLocaleString()}</div>}
      ...
      <div style={{ display:'flex', flexDirection:'column', borderLeft: rightPlayers.length > 0 ? '1px solid rgba(255,255,255,0.05)' : 'none', background:'rgba(0,0,0,0.3)', overflow:'hidden' }}>
        {rightPlayers.map(a => (<div key={a.nickname} style={{ flex:1, borderBottom:'1px solid rgba(255,255,255,0.04)' }}><Spotlight player={players[a.nickname]} instrument={a.instrument} color={INSTRUMENT_COLORS[a.instrument]||'#fff'} icon={INSTRUMENT_ICONS[a.instrument]||'🎵'} side="right" /></div>))}
      </div>
      <div style={{ gridColumn:'1 / -1', display:'flex', alignItems:'center', justifyContent:'space-between', padding:'6px 20px', background:'#0a0a14', borderTop:'1px solid rgba(255,255,255,0.05)', fontSize:'clamp(9px,0.9vw,12px)', color:'rgba(255,255,255,0.25)' }}>
        <div style={{ display:'flex', alignItems:'center', gap:'6px' }}><div style={{ width:7, height:7, borderRadius:'50%', background: connected ? '#39ff84' : '#ff3b5c' }} />{connected ? 'CONNESSO' : 'OFFLINE'}</div>
